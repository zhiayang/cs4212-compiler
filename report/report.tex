...
submissions --- notably \eqtt{null} acting like \eqtt{String} (but not \ttt{null + null}), and double-quote escapes.
...
% end subsubsection (Multiplication by a Constant)
...
% end subsubsection (Strings & Large Constants)
...
% end subsubsection (Dot-op Assignments)
...
% end subsubsection (Function Calls)
...
(I'll revisit this example later as well). The predecessors of \eqtt{check} are actually both \eqtt{body} and \eqtt{init},
so \enquote{normal} dataflow analysis sees that \eqtt{x} is modified, and so it refuses to propagate the constant into the
...
% end subsubsection (Integer Division)


\subsubsection{Function Calls}

Function calls needed a little special consideration, apart from the mess with passing parameters on the stack. As
the calling convention dictates, \ttt{a1 - a4} are clobbered on a call boundary, so they need to be saved, but clearly it
...
\enquote{register live range}, or ranges (statements) where each \itl{register} is live. Then, in deciding whether or not
one of the \eqtt{a} registers should be saved, we check if it is live in any of the \itl{successors} to the call statement.
...
However, I have yet to counter the situation where (for example, \eqtt{a1}) is not spilled, and it is used before
...
I think mitigating this would require dataflow at the \itl{register} level (instead of variable-level), which
I am not keen to do.
...
Somewhat related to memory, but not really --- as mentioned above, the compiler makes use of the \enquote{constant pool}
to store large constants inline with the code, eg. \mintinline{asm}{ldr a1, =#12345}. The issue is that the assembler
appears to \enquote{pool} (haha) these constants near the end of the generated executable (maybe in the data section?).

Apparently, the maximum offset is at most \qty{4}{\kilo\byte}; if functions get too large, or there are too many of them,
the assembler complains. The mitigation is to insert \mintinline{asm}{.ltorg} in between functions, so the assembler can
choose to place constants there as well.
...
\eqtt{check1} has only 1 predecessor (the init), so the constant \ttt{x = 10} can definitely be propagated to the first loop
condition. This allows the branch to be eliminated, thereby making \eqtt{check2} and \eqtt{body} unreachable. This way,
...
\begin{center}
\tikzstyle{block} = [rectangle, minimum width=3cm, minimum height=1cm, line width=0.1mm, text centered, draw=black]
\tikzstyle{arrow} = [thick,-{Stealth[length=2.3mm,width=1.8mm]}]

\begin{tikzpicture}[node distance=20mm]
	\node (entry)  [block] {init};
	\node (check1) [block,below of=entry] {check1};
	\node (body)   [block,right of=check1,xshift=30mm] {body};
	\node (check2) [block,below of=body]   {check2};
	\node (end)    [block,below of=check1] {end};

	\draw [arrow] (entry)  -- (check1);
	\draw [arrow] (check1) -- (body);
	\draw [arrow] (check1) -- (end);

	\draw [arrow] (check2) -- (end);

	\draw [arrow] (check2) to [out=120,in=-120] (body);
	\draw [arrow] (body) to [out=-60,in=60] (check2);
\end{tikzpicture}
\end{center}

The issue here is that \eqtt{body} and \eqtt{check2} are mutual predecessors; with the dataflow meet being an intersection,
a constant will never propagate past that pair (unless it is already present in both of them, which isn't possible), so
this construction breaks propagation of all constants from \eqtt{init} to \eqtt{end}.




% end subsubsection (Loop Elimination)
